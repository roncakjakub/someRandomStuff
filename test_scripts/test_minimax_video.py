"""
Test script for Minimax Hailuo 2.3 video generation.
Tests the fixes for Issue #3: Aspect ratio consistency.
"""
import sys
...
def test_minimax_video():
    """Test Minimax video generation with 9:16 aspect ratio."""
    print("=" * 60)
    print("Testing Minimax Hailuo 2.3 Video Generation")
    print("=" * 60)
    ...
    image_result = image_tool.run({
        "prompt": "A person dancing energetically, dynamic movement, photorealistic",
    ...
    # Step 2: Test Minimax video generation
    print("\n[2/3] Testing Minimax Hailuo 2.3 API...")
    print("Key insight:")
    print("  - Minimax inherits aspect ratio from input image")
    print("  - No explicit aspect_ratio parameter")
    print("  - Input image MUST be 9:16 for output to be 9:16")
    
    video_tool = MinimaxVideoTool()
    
    try:
        video_result = video_tool.run({
            "image_path": image_path,
            "prompt": "The person dances with fluid movements, camera follows the action",
            "output_dir": "output/test"
        })
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if abs(aspect - expected_aspect) < 0.01:
                        ...
                        print("✅ Minimax correctly inherited 9:16 from input image!")
                    else:
